...
Use these after the README and hosted-access page are live. The hosted MCP access request page has been created and tested.
...
- If your workflow specifically needs hosted MCP, use the self-service request page to get a unique access token.
...
4/ If your workflow specifically depends on hosted MCP, use the self-service request page to get a unique access token.
...
I am documenting the free local options clearly and providing a self-service hosted MCP access page.

The access page is intentionally short. Email is the only required field so the service can send the token. Optional context must not include student data or confidential school information.
