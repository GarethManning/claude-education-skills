...
Hosted MCP access now requires a unique auth token. Request one here: [Hosted MCP access signup](https://mcp-server-sigma-sooty.vercel.app/request-access). The service sends the MCP URL, token, and short setup instructions by email. See [Hosted MCP access](docs/HOSTED_MCP_ACCESS.md) for details.
